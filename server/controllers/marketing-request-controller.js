...
import {MDRSubmission} from "../model/submission";
...
@controller("/api/marketing-request")
class MarketingRequestCtrl extends ExpressController {
    ...
    async submitFormToSuppLog(submissionID, canSubmitToSlack = true) {
        ...
        if (parsedResp.responseCode != 200) {
            ...
            try {
                formRule = MarketingRequestRules.A;
                newRow = {
                    range: formRule.sheet,
                    rows: [applySheetRule(formRule.rule, parsedForm)]
                };

                results.username = applySheetRule([13], parsedForm)[0];
...
export default MarketingRequestCtrl;
